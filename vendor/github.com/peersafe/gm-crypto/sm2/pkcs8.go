...
func ParseSm2PublicKey(der []byte) (*PublicKey, error) {
	...
	fullBytes := []byte{6, 8, 42, 129, 28, 207, 85, 1, 130, 45}
	if bytes.Compare(pubkey.Algo.Parameters.FullBytes, fullBytes) != 0 {
		return nil, errors.New("x509: not sm2 public key.")
	}
...
func MarshalSm2PublicKey(key *PublicKey) ([]byte, error) {
	...
	if key.Curve != sm2P256 {
		return nil, errors.New("x509: not sm2 key")
	}
